...
```{r Model 1: Internal Features Only}
head(boulder.sf)
...
reg.cv <- 
  train(price ~ ., data = st_drop_geometry(bouldert.sf) %>% 
                                dplyr::select(price, Size, builtYear, TotalBedrooms,                                                  TotalBathrooms, parks_nn1, parks_nn2), 
     method = "lm", trControl = fitControl, na.action = na.pass)
...
reg.tracts <- lm(price ~ ., data = as.data.frame(boulder.training) %>% 
                                  dplyr::select(price, Size, builtYear, 
                                               ...
                                               parks_nn1, university_nn1, marijuanaestablishments_nn1))
...
tracts19 <- tracts19 %>% 
         mutate (incomeContext = ifelse(MedHHInc > 32322, "High Income", "Low Income"))
