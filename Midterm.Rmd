...
#filter boulder.sf with specific variables of interest
...
#Nearest neighbor distance for Parks
st_c <- st_coordinates
...
boulder.sf <-
  boulder.sf  %>% 
    mutate(
      uni_nn1 = nn_function(st_c(boulder.sf), st_c(uni.sf), 1),
      uni_nn2 = nn_function(st_c(boulder.sf), st_c(uni.sf), 2), 
      uni_nn3 = nn_function(st_c(boulder.sf), st_c(uni.sf), 3), 
      uni_nn4 = nn_function(st_c(boulder.sf), st_c(uni.sf), 4), 
      uni_nn5 = nn_function(st_c(boulder.sf), st_c(uni.sf), 5)) 
...
boulder.sf <-
  boulder.sf  %>% 
    mutate(
      restaurants_nn1 = nn_function(st_c(boulder.sf), st_c(restaurant.sf), 1),
      restaurants_nn2 = nn_function(st_c(boulder.sf), st_c(restaurant.sf), 2), 
      restaurants_nn3 = nn_function(st_c(boulder.sf), st_c(restaurant.sf), 3), 
      restaurants_nn4 = nn_function(st_c(boulder.sf), st_c(restaurant.sf), 4), 
      restaurants_nn5 = nn_function(st_c(boulder.sf), st_c(restaurant.sf), 5)) 
...
#Internal Characteristics Corr Plots
st_drop_geometry(boulder.sf) %>% 
  ...
  ggplot(aes(Value, price)) +
     ...
     plotTheme()
```
In the plot above, we see how price increases as age decreases, and how the number of bedrooms, overall size and garage space are positively correlated to price increase. Looking at non numeric attributes of the home might shed better light into understanding why age is negatively correlated to price. A general assumption here could be that architectural styles in this County are not historically valuable to appraisers. Meaning that age may not add historical value to a property as much as the other commodities do. 
...
st_drop_geometry(boulder.sf) %>% 
  ...
  ggplot(aes(Value, price)) +
     ...
     plotTheme()
```
This is a second correlation plot though, shows other specific commodities such as main floor size and total bathrooms can positively influence price, while infrastructure such as Ac or Heating might negatively influence price. 
