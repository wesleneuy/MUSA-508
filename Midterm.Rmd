...
boulder.sf <- housingData %>%
 ...
 #select(boulder.sf, price, pricesqft, Size, TotalBedrooms, GarageSize, 
                #BuildingType, Quality, YearBuilt, Material, ToPredict)
...
boulder.sf <-
  boulder.sf  %>% 
    mutate(
      marijuanaesttablishments_nn1 = nn_function(st_c(boulder.sf), st_c(marijuanaestablishments), 1),
...
corrplot <- st_drop_geometry(boulder.sf) %>% 
  select(price, pricesqft, TotalBathrooms, Size, TotalBedrooms, GarageSize, MedHHInc, TotalPop, parks_nn1, university_nn1, trails_nn1, schools_nn1, healthcare_nn1, )), ) %>% na.omit(na.action = "omit")
