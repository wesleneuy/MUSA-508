...
uni.sf <- st_geometry(uni$osm_points) %>%
  st_transform(4326) %>%
  st_sf() %>%
  cbind(., uni$osm_points$osm_id) %>% #cbind is giving me an error, not sure how to fix it: arguments imply differing number of rows: 400, 0.
...
```{r Distance to Roads echo=FALSE, message=FALSE, warning=FALSE}
...
MRBuffer <-
  st_join(
    st_centroid(dplyr::select(boulder.sf, geometry, price, builtYear)),
    multipleRingBuffer(Unioned_buffer,10000,26400)) %>%
  st_drop_geometry() %>%
  left_join(dplyr::select(boulder.sf, price, builtYear),
            by=c("price"="price")) %>%
  st_sf() %>%
  replace_na(list(distance = 0))%>% 
  mutate(distance = distance+26400)

boulder.sf <- 
  rbind(
    st_centroid(boulder.)[buffer,] %>%
      st_drop_geometry() %>%
      left_join(allTracts) %>%
      st_sf() %>%
      mutate(TOD = "TOD"),
