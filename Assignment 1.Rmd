...
ggplot() +
  ...
  geom_sf(data=Bostonbdry,fill = "transparent",color="#b5838d",size=0.5)+
...
#This would always give me an error - Error in dplyr::select(GEOID, Crimecount, MedRent, TOD, MedRent.inf, year) : object 'GEOID' not found
Crime_MRBuffer <-
  st_join(
  ...
  replace_na(list(distance = 0))%>% 
  mutate(distance = distance+804.672)
#Because the buffer would not work, then this doesnt work either
CrimeSum <- 
  Crime_MRBuffer %>%
  st_drop_geometry() %>%
  group_by(year,distance)%>%
...
#Alternative to using the crimedatapackage: 
