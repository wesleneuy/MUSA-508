...
No mortgage, no tax lien, full time residence in Philly - will accept tax credit
...
```{r cars, echo=TRUE, message=FALSE, warning=FALSE, fig.height=8}
...
```{r kitchen sick model, echo=TRUE, message=FALSE, warning=FALSE}
...
cost_benefit_table %>%
  kable(caption = "Figure X: Cost/Benefit Table") %>%
  kable_styling()
...
This shows thresholds as function of total revenuw and count of people who took the credit
...
whichThreshold_revenue <- 
  ...
  group_by(Threshold) %>% 
  summarize(Total_Revenue = sum(Revenue), Total_Count_Of_Credits = su'm(TookCredit))
