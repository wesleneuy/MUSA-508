...
#Did not run this regression
reg1 <- glm(y_numeric ~ .,
                  data=housingTrain %>% 
                    dplyr::select(-age, marital, -mortgage, -contact, -day_of_week, -pdays, -y, -unemploy_rate, -education), family="binomial" (link="logit"))

#Used this instead
reg1 <- glm(y_numeric ~ .,
                   data=housingTrain %>% 
                   dplyr::select(-cons.price.idx, -cons.conf.idx, -y, -spent_on_repairs, -previous), family="binomial" (link="logit"))
...
testProbs = data.frame(observed = as.factor(housingTest$y_numeric),
                       probs = predict(reg1, housingTest, type = 'response'))
...
kable(optimalthreshold_table,
       caption = "Cost/Benefit Table") %>% kable_styling()
>>>>>>> 683d31020b99ea760cf3fb52a944223d7050ee3f
```