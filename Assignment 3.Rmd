...
#Race context
```{r by race, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r by kernel density, echo=TRUE, message=FALSE, warning=FALSE}
...
rbind(crash_KDE_sf, crash_risk_sf) %>%
  ...
  ggplot() +
    ...
    geom_sf(data = sample_n(traffic_crash19, 3000), size = .5, colour = "black") +
    facet_wrap(~label, ) +
...
Unfortunately, our model captures risk predictions below the kernel density at the two higher risk categories, making it an unreliable source to predict crashes. 
