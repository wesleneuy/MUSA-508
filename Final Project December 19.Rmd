...
```{r Ridership Plot, echo=TRUE, message=FALSE, warning=FALSE}

#make a plot of ridership per station (check homework 1)
ggplot() +
   geom_sf(data=njCensus, fill="grey09") +
   geom_point(data=NECstops.sf, aes(size=boardings), fill="#CA3697", legend=TRUE) +
...
dat1$to <- factor(dat1$to,                                    
                  levels = c("Trenton","Hamilton","Princeton Junction", "Jersey Avenue", "New Brunswick",
                             "Edison","Metuchen", "Metropark", "Rahway", "Linden", "Elizabeth", "North 
                             Elizabeth", "Newark Airport", "Newark Penn Station", "Secaucus Upper Lvl", 
...
dat1 %>% #filter(direction=="Northbound", month==1, day==1, train_id=="3806")%>%
  ...
  geom_bar(stat="identity", fill="#03045e") +
  labs(x="Station",
       y="Average Delay Minutes",
...
stations <- dat_all %>%
  group_by(to) %>%
  summarize(mean_delay = mean(delay_minutes)) #needs to be an sf



ggplot() +
  geom_sf(data = njCensus, fill="grey90") +
  geom_sf(data = dat_all, aes(color=mean_delay), size=1, show.legend = "point") +
...
# Weather Data: How good of a predictor can this external characteristic be?
...
We also looked at the correlation between weather features and average delay time. At lower temperatures, the delay times are longer. For other variables, the relationship isn't very clear from the visualizations. 
...
```{r FE-2, echo=TRUE, message=FALSE, warning=FALSE}
...
panelnoGEOM <- train.weather.panel.lags.final %>% 
  mutate(y6_numeric = ifelse(mean_delay < minutes(6), 0, 1),
         y6 = ifelse(mean_delay < minutes(6), "no", "yes"))
...
panelGEOM <- left_join(panelnoGEOM, NECstops) %>%
  st_as_sf(crs =3424, agr = "constant")


panelnoGEOM[c("onestoplag", "twostoplag", "onestopearlieron", "twostopearlieron", "y6_numeric")][is.na(panelnoGEOM[c("onestoplag", "twostoplag", "onestopearlieron", "twostopearlieron","y6_numeric")])] <- 0
...
reg1 <- lm(mean_delay ~ Temperature + Precipitation,  data=panelnoGEOM)
